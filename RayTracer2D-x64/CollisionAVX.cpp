...
__m256 _RayLineVsSegmentsAVX(const Ray& ray, const std::vector<Segment>& segments, int index_first_segment)
{
	...
	__m256 _ray_origin_x, _ray_origin_y, _segments_p1_x, _segments_p1_y, _segments_p2_x, _segments_p2_y;
	_ray_origin_x = _mm256_set1_ps(ray.origin.x);
	_ray_origin_y = _mm256_set1_ps(ray.origin.y);
	int i = index_first_segment;
	_segments_p1_x = _mm256_set_ps(
		segments[i].p1.x, segments[i + 1].p1.x,
		segments[i + 2].p1.x, segments[i + 3].p1.x,
		segments[i + 4].p1.x, segments[i + 5].p1.x,
		segments[i + 6].p1.x, segments[i + 7].p1.x
	);
	_segments_p1_y = _mm256_set_ps(
		segments[i].p1.y, segments[i + 1].p1.y,
		segments[i + 2].p1.y, segments[i + 3].p1.y,
		segments[i + 4].p1.y, segments[i + 5].p1.y,
		segments[i + 6].p1.y, segments[i + 7].p1.y
	);
	_segments_p2_x = _mm256_set_ps(
		segments[i].p2.x, segments[i + 1].p2.x,
		segments[i + 2].p2.x, segments[i + 3].p2.x,
		segments[i + 4].p2.x, segments[i + 5].p2.x,
		segments[i + 6].p2.x, segments[i + 7].p2.x
	);
	_segments_p2_y = _mm256_set_ps(
		segments[i].p2.y, segments[i + 1].p2.y,
		segments[i + 2].p2.y, segments[i + 3].p2.y,
		segments[i + 4].p2.y, segments[i + 5].p2.y,
		segments[i + 6].p2.y, segments[i + 7].p2.y
	);

	__m256 _ray_origin_to_p1_x, _ray_origin_to_p1_y, _ray_origin_to_p2_x, _ray_origin_to_p2_y;
	...
	_ray_origin_to_p2_x = _mm256_sub_ps(_segments_p2_x, _ray_origin_x);
	_ray_origin_to_p2_y = _mm256_sub_ps(_segments_p2_y, _ray_origin_y);

	__m256 _ray_direction_x, _ray_direction_y;
	_ray_direction_x = _mm256_set1_ps(ray.direction.x);
	_ray_direction_y = _mm256_set1_ps(ray.direction.y);

	__m256 _ray_origin_to_p1_ray_direction_x1_times_y2, _ray_origin_to_p1_ray_direction_x2_times_y1,
		_ray_origin_to_p2_ray_direction_x1_times_y2, _ray_origin_to_p2_ray_direction_x2_times_y1;
	__m256 _ray_origin_to_p1_ray_direction_cross_product, _ray_origin_to_p2_ray_direction_cross_product;
	__m256 _cross_products_multiplied;
	__m256 _all_zeroes = _mm256_set1_ps(0.0f);
	__m256 _ray_points_towards_surface;

	_ray_origin_to_p1_ray_direction_x1_times_y2 = _mm256_mul_ps(_ray_origin_to_p1_x, _ray_direction_y);
	_ray_origin_to_p1_ray_direction_x2_times_y1 = _mm256_mul_ps(_ray_origin_to_p1_y, _ray_direction_x);
	_ray_origin_to_p2_ray_direction_x1_times_y2 = _mm256_mul_ps(_ray_origin_to_p2_x, _ray_direction_y);
	_ray_origin_to_p2_ray_direction_x2_times_y1 = _mm256_mul_ps(_ray_origin_to_p2_y, _ray_direction_x);

	_ray_origin_to_p1_ray_direction_cross_product = _mm256_sub_ps(_ray_origin_to_p1_ray_direction_x1_times_y2, _ray_origin_to_p1_ray_direction_x2_times_y1);
	_ray_origin_to_p2_ray_direction_cross_product = _mm256_sub_ps(_ray_origin_to_p2_ray_direction_x1_times_y2, _ray_origin_to_p2_ray_direction_x2_times_y1);

	_cross_products_multiplied = _mm256_mul_ps(_ray_origin_to_p1_ray_direction_cross_product, _ray_origin_to_p2_ray_direction_cross_product);

	_ray_points_towards_surface = _mm256_cmp_ps(_cross_products_multiplied, _all_zeroes, _CMP_LE_OQ);

	return _ray_points_towards_surface;
}
...
CollisionInfoAVXRegisters _RayVsSurfaceAVX(const Ray& ray1, const std::vector<Surface>& surfaces2, int index_first_surface, __m256& _inter_x, __m256& _inter_y)
{
	/*
		{ a1 * x + b1 = y
		{ a2 * x + b2 = y
	*/
	...
	int j = index_first_surface;
	_l1p1x = _mm256_set1_ps(ray1.origin.x);
	_l1p1y = _mm256_set1_ps(ray1.origin.y);
	_l1p2x = _mm256_set1_ps(ray1.EndPoint().x);
	_l1p2y = _mm256_set1_ps(ray1.EndPoint().y);
	_l2p1x = _mm256_set_ps(surfaces2[j].p1.x, surfaces2[j + 1].p1.x,
		surfaces2[j + 2].p1.x, surfaces2[j + 3].p1.x,
		surfaces2[j + 4].p1.x, surfaces2[j + 5].p1.x,
		surfaces2[j + 6].p1.x, surfaces2[j + 7].p1.x);
	_l2p1y = _mm256_set_ps(surfaces2[j].p1.y, surfaces2[j + 1].p1.y,
		surfaces2[j + 2].p1.y, surfaces2[j + 3].p1.y,
		surfaces2[j + 4].p1.y, surfaces2[j + 5].p1.y, 
		surfaces2[j + 6].p1.y, surfaces2[j + 7].p1.y);
	_l2p2x = _mm256_set_ps(surfaces2[j].p2.x, surfaces2[j + 1].p2.x,
		surfaces2[j + 2].p2.x, surfaces2[j + 3].p2.x,
		surfaces2[j + 4].p2.x, surfaces2[j + 5].p2.x, 
		surfaces2[j + 6].p2.x, surfaces2[j + 7].p2.x);
	_l2p2y = _mm256_set_ps(surfaces2[j].p2.y, surfaces2[j + 1].p2.y,
		surfaces2[j + 2].p2.y, surfaces2[j + 3].p2.y,
		surfaces2[j + 4].p2.y, surfaces2[j + 5].p2.y, 
		surfaces2[j + 6].p2.y, surfaces2[j + 7].p2.y);
	...
	_l2extension = _mm256_set_ps(surfaces2[j].extension, surfaces2[j + 1].extension,
		surfaces2[j + 2].extension, surfaces2[j + 3].extension,
		surfaces2[j + 4].extension, surfaces2[j + 5].extension,
		surfaces2[j + 6].extension, surfaces2[j + 7].extension);
	_l2_limit_dot_product = _mm256_add_ps(_l2extension, _l2length);
	_l2_limit_dot_product = _mm256_mul_ps(_l2_limit_dot_product, _l2extension);
